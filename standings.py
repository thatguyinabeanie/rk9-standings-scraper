...
def main_worker(directory, link, get_decklists, get_roster, output_dir):
    last_page_loaded = ""
    ...
    for standing in standings:
            ...
            for iRounds in range(rounds_from_url):
                ...
                for match_data in matches:
                    ...
                    if p1 is None:
                        if len(player1_name) > 0:
                            ...
                            if p1.country == "" and roster is not None:
                                p1.country = roster.get_country(p1)
                    ...
                    if p2 is None:
                        if len(player2_name) > 0:
                            ...
                            if p2.country == "" and roster is not None:
                                p2.country = roster.get_country(p2)
                ...
                if decklists_players:
                    for player in standing.players:
                        deck_index = -1
                        pos = -1
                        for dl_player in decklists_players.players:
                            pos = pos + 1
                            if ((dl_player.name.upper() == player.name.upper()
                                    or remove_country(dl_player.name).upper() == remove_country(player.name).upper())
                                    and dl_player.level == player.level):
                                deck_index = pos
                                break
                        if deck_index != -1:
        ...
        if decklists_players:
            for player in standing.players:
                ...
                for dl_player in decklists_players.players:
                    pos = pos + 1
                    if ((dl_player.name == player.name or remove_country(dl_player.name) == player.name)
                            and dl_player.level == player.level):
                        deck_index = pos
                        break
                if deck_index != -1:
...
if __name__ == "__main__":
    ...
    """
    main_worker(args.id, args.url, args.decklists, args.roster, args.output_dir)
